{
  name: "brawny gigas shield-maiden",
  noun: "shield-maiden",
  url: "https://gswiki.play.net/Brawny_gigas_shield-maiden",
  picture: "",
  level: 106,
  family: "gigas",
  type: "biped",
  undead: false,
  blood: true,
  bones: nil,
  limbs: nil,
  witherable: true,
  sympathy: true,
  muggable: true,
  sleepable: nil,
  boss: false,
  boss_type: nil,
  otherclass: [],
  areas: [
    {
      name: "Hinterwilds",
      uids: [7503301..7503312, 7503321..7503332, 7503341..7503353, 7503361..7503366, 7503371..7503374, 7503381..7503384]
    }
  ],
  bcs: true,
  max_hp: 900,
  speed: nil,
  height: 30,
  size: "huge",
  attack_attributes: {
    physical_attacks: [
      {
        name: "shield strike",
        as: (525..566)
      },
      {
        name: "spear",
        as: (525..590)
      },
      {
        name: "Charge",
        as: 529
      },
      {
        name: "Enormous tusks",
        as: (529..538)
      },
      {
        name: "Golden targe",
        as: (536..590)
      },
      {
        name: "Immense fel-hafted handaxe",
        as: (540..566)
      },
      {
        name: "Lunge",
        as: (524..570)
      }
    ],
    bolt_spells: [],
    warding_spells: [
      {
        name: "aura of the arkati (1614)",
        cs: (440..482)
      },
      {
        name: "judgment (1630)",
        cs: (443..482)
      },
      {
        name: "Shield Bash",
        cs: 443
      }
    ],
    offensive_spells: [],
    maneuvers: [
      {
        name: "shield bash",
      },
      {
        name: "shield push"
      },
      {
        name: "shield trample"
      }
    ],
    special_abilities: [],
    special_notes: []
  },
  defense_attributes: {
    asg: "17",
    immunities: [],
    melee: (272..770),
    ranged: (247..565),
    bolt: (247..565),
    udf: (490..878),
    bar_td: (377..380),
    cle_td: (387..476),
    emp_td: (411..455),
    pal_td: (362..371),
    ran_td: (311..356),
    sor_td: (428..438),
    wiz_td: nil,
    mje_td: (452..462),
    mne_td: (452..462),
    mjs_td: (390..408),
    mns_td: (390..408),
    mnm_td: nil,
    defensive_spells: [
      {
        name: "divine shield (1609)"
      },
      {
        name: "dauntless (1606)"
      }
    ],
    defensive_abilities: [
      {
        name: "damage resistance",
        note: "able to gain resistance to damage type attacked with"
      }
    ],
    special_defenses: []
  },
  special_other: "",
  abilities: [],
  alchemy: [],
  equipment: [
    "a gilded steel breastplate adorned with faint scrollwork",
    "a gold-tipped heavy spear",
    "a riveted golden targe"
  ],
  treasure: {
    coins: true,
    magic_items: true,
    gems: true,
    boxes: true,
    skin: false,
    other: [
      "gigas fragment",
      "ayanad crystal",
      "n'ayanad crystal",
      "petrified mammoth tusk"
    ],
    blunt_required: false,
    armaments: [
      "drake falchion",
      "drake greatsword",
      "drake mace",
      "feras mattock",
      "drake greataxe"
    ],
    transmogs: nil
  },
  messaging: {
    description: "Voluptuous and sturdy of build, the gigas shield-maiden has the musculature of a seasoned fighter and the cool gaze of a practiced tactician.  Towering at nearly thirty feet in height, the shield-maiden moves with a dangerous grace.  The armor {pronoun} wears is not ornate, but it shines like hammered gold and barely makes a sound as {pronoun} moves.",
    arrival: [
      "A brawny gigas shield-maiden marches in with the grace of a seasoned warrior.",
      "A brawny gigas shield-maiden marches into the area, grim purpose written across {pronoun} face.",
      "Preceded by a mournful dirge, a grim gigas skald stalks in, {pronoun} song accompanied by the clacking of the crude jeweled beads adorning the ceremonial garb that {pronoun} wears.",
      "A brawny gigas shield-maiden marches in, {pronoun} back arched and {pronoun} movements stiff from pain.",
      "A brawny gigas shield-maiden rides {target} in, swaying with every heavy footfall.",
      "A brawny gigas shield-maiden marches into the room, grim purpose written across {pronoun} face.",
      "A brawny gigas shield-maiden rides {target} in, {target} limping with every great step.",
      "A brawny gigas shield-maiden just came through a breached red stone wall.",
      "A brawny gigas shield-maiden just arrived, looking terrified."
    ],
    flee: [
      "Warily scanning the area for threats, a brawny gigas shield-maiden marches {direction}.",
      "Keeping {pronoun} head held high in spite of {pronoun} wounds, a brawny gigas shield-maiden struggles {direction}.",
      "A brawny gigas shield-maiden just went into a thatched timber smithy.",
      "Amber radiance flickers around a brawny gigas shield-maiden's corpse, swelling into a golden pillar of light that is blinding to behold.  When the brilliance retreats, the shield-maiden's body is no more.",
      "A brawny gigas shield-maiden just went into a huge hut.",
      "A brawny gigas shield-maiden just went into a huge hoarbeam longhouse.",
      "A brawny gigas shield-maiden just went into a long timber hall.",
      "A brawny gigas shield-maiden rides {target} out, swaying with every heavy footfall."
    ],
    spell_prep: [
      "A brawny gigas shield-maiden raises a fist to the heavens as {pronoun} eyes begin to glow like molten gold.",
      "A brawny gigas shield-maiden's eyes begin to glow purple.",
      "A brawny gigas shield-maiden raises {pronoun} eyes to the heavens as sunny light enshrouds {pronoun}!"
    ],
    death: [
      "A plaintive look passes across a brawny gigas shield-maiden's eyes like a fleeting shadow as {pronoun} goes still in death.",
      "A tattooed gigas berserker's fists tense with impotent rage as {pronoun} surrenders to death.",
      "A grim gigas skald raises a hand as if to grasp for support as {pronoun} collapses, life going out of {pronoun} form."
    ],
    decay: [
      "Rot consumes a brawny gigas shield-maiden's body, leaving little behind."
    ],

    stand: [
      "A brawny gigas shield-maiden rises fluidly, dusting a bit of dirt from one knee with a faint smirk."
    ],
    attacks: {
      attack: [
        "In a display of martial precision, a brawny gigas shield-maiden thrusts with a gold-tipped heavy spear at you!",
        "A brawny gigas shield-maiden slams a fist down at you with earthshattering force!",
        "In a display of martial precision, a brawny gigas shield-maiden thrusts with an immense fel-hafted handaxe at you!",
        "A heavily armored battle mastodon tries to spear you with {pronoun} enormous tusks!",
        "Froth bubbling on {pronoun} lips, a tattooed gigas berserker swings {weapon} at you in a murderous arc!",
        "A brawny gigas shield-maiden leaps from the back of {target} as {target} topples, narrowly avoiding being pinned beneath {target} mount!",
        "A brawny gigas shield-maiden lashes at you with an awesome shield bash!",
        "A brawny gigas shield-maiden lashes at you with a half-hearted shield bash!",
        "A brawny gigas shield-maiden lashes at you with a light shield bash!",
        "The brawny gigas shield-maiden slams into {target}, who is sent careening headlong into a nearby group of combatants as {pronoun} falls to the ground!",
        "The brawny gigas shield-maiden slams into you, and you are sent careening headlong into a nearby group of combatants as you fall to the ground!",
        "A brawny gigas shield-maiden lashes out with a solid shield bash!",
        "A brawny gigas shield-maiden lashes out with a sharp shield bash!",
        "A brawny gigas shield-maiden lashes out with a competent shield bash!",
        "A brawny gigas shield-maiden lashes out with a mediocre shield bash!",
        "A brawny gigas shield-maiden rushes at {target} in a thundering charge!",
        "A brawny gigas shield-maiden lashes at you with a mediocre shield bash!",
        "A brawny gigas shield-maiden lashes out with a quick shield bash!",
        "The brawny gigas shield-maiden slams into you, and you are sent careening into {target} as you fall to the ground!",
        "A brawny gigas shield-maiden lashes at you with a sharp shield bash!",
        "A brawny gigas shield-maiden lashes out with a half-hearted shield bash!",
        "A brawny gigas shield-maiden lashes out with a strong shield bash!",
        "A brawny gigas shield-maiden lashes at you with a solid shield bash!",
        "A brawny gigas shield-maiden lashes out with a fair shield bash!",
        "A brawny gigas shield-maiden lashes out with a terrific shield bash!",
        "A brawny gigas shield-maiden lashes at you with a quick shield bash!",
        "A brawny gigas shield-maiden lashes out with a light shield bash!",
        "The brawny gigas shield-maiden slams into {target}, who is sent careening into {target} as {pronoun} falls to the ground!",
        "A brawny gigas shield-maiden lashes at you with a competent shield bash!",
        "A brawny gigas shield-maiden lashes out with a skilled shield bash!",
        "A brawny gigas shield-maiden lashes at you with a strong shield bash!",
        "A brawny gigas shield-maiden lashes out with a crushing shield bash!",
        "A brawny gigas shield-maiden lashes at you with a fair shield bash!",
        "A brawny gigas shield-maiden lashes out with an awesome shield bash!",
        "A brawny gigas shield-maiden lashes at you with a terrific shield bash!",
        "A brawny gigas shield-maiden lashes at you with a slamming shield bash!",
        "A brawny gigas shield-maiden lashes at you with a lucky shield bash!",
        "A brawny gigas shield-maiden lashes out with a slamming shield bash!",
        "A brawny gigas shield-maiden lashes out with a lucky shield bash!",
        "A brawny gigas shield-maiden lashes at you with an expert shield bash!",
        "A brawny gigas shield-maiden lashes at you with a skilled shield bash!",
        "A brawny gigas shield-maiden lashes at you with a hard shield bash!",
        "A brawny gigas shield-maiden brings {pronoun} fists together in a martial gesture toward you!",
        "A brawny gigas shield-maiden raises a fist to {pronoun} chest and bows down at you."
      ],
      damage_resistance: [
        "In response to the vibrations, a brawny gigas shield-maiden's skin seems to discolor and harden, lending the shield-maiden unnatural durability!"
      ],
      shield_bash: [
        "A brawny gigas shield-maiden lunges forward at you with {pronoun} golden targe and attempts a shield bash!",
        "A brawny gigas shield-maiden launches a quick bash with {pronoun} golden targe at you!"
      ],
      shield_push: [
        "A brawny gigas shield-maiden raises {pronoun} golden targe and attempts to push you away!"
      ],
      shield_strike: [
        "A brawny gigas shield-maiden launches a quick bash with {pronoun} golden targe at you!"
      ],
      shield_trample: [
        "A brawny gigas shield-maiden raises her golden targe and charges headlong towards you!"
      ],
      spear: [
        "In a display of martial precision, a brawny gigas shield-maiden thrusts with a gold-tipped heavy spear at you!"
      ]
    },
    info: {
      general: [
        "* Shield-maidens are square creatures, so players of semis and pures can take advantage of their low TD by casting CS-based offensive spells.\n* Shield-maidens have a pretty high rate of blocking physical attacks with their shields when unhindered, so setup abilities like Sunder Shield, Aura of the Arkati (1614), anything that inflicts Blinded, anything that knocks prone, and so forth are recommended for primarily physical combatants. Alternatively, unarmed combat can't be blocked."
      ],
      class_tips: {
        cleric: [],
        paladin: [],
        ranger: [],
        bard: [
          "* Vibration Chant (1002) works twice and will often leave them helpless or dead."
        ],
        wizard: [
          "* Shield-maidens can be targeted with Mana Leech (516)."
        ],
        empath: [],
        rogue: [],
        warrior: [],
        sorcerer: []
      },
      miscellany: []
    },
  }
}

=begin
# mount
A brawny gigas shield-maiden gets a running start and then acrobatically leaps up onto the back of a heavily armored battle mastodon, mounting it!

# reveal
A brawny gigas shield-maiden glowers disdainfully as she sweeps the surroundings with her gaze.
A brawny gigas shield-maiden points forcefully into the shadows, revealing you in your hiding place!

# damage resistance
In response to the vibrations, a brawny gigas shield-maiden's skin seems to discolor and harden, lending the shield-maiden unnatural durability!
A brawny gigas shield-maiden is unharmed by the impact!
A brawny gigas shield-maiden is unharmed by the impact!
A brawny gigas shield-maiden is unharmed by the impact!

=end
