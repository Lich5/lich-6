...
RSpec.describe 'CI foundation guard scripts' do
  ...
  it 'wires the real CI commands, artifacts, and pull-request head SHA' do
    ...
    expect(workflow).to include('test "$ruby_path" = "$rbenv_root/shims/ruby"')
    expect(workflow).to include('run: rbenv exec rspec')
    expect(workflow).to include('run: rbenv exec ruby script/ci/check_shim_namespace.rb')
    expect(workflow).to include("run: rbenv exec rubocop --only Custom/AsciiOnlySource")
    expect(workflow).to include('run: rbenv exec ruby script/ci/startup_load_check.rb')
